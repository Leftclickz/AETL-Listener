	...
	while (!ProgramExecutionComplete)
	{
		...
		if (isFolderLocked == false)
		{
			...
			else
			{
				...
				VideoRenderUpdateThread->detach();
#endif			
...
void VideoRenderUpdateLoop()
{
	...
	//otherwise we fetch a file. 
		...
		//If theres no files that fit the bill we need to get out.
		if (Project::TIMESTAMPED_FILENAME == "")
		{
			return;
		}
	}
	...
	TERMINATE_IF_FAILURE(FetchSQLBuildLogUnsafe, nullptr, nullptr, LogFile::ATTACHED_PROJECT);
	...
	LogFile::WriteToLog("Creating project sub folders.");
	{
		...
		TERMINATE_IF_FAILURE(CreateDirectoryUnsafe, &dir, nullptr, LogFile::ATTACHED_PROJECT);
	...
	//create our new project directory and rename the file
	{
		...
		TERMINATE_IF_FAILURE(CreateDirectoryUnsafe, &currentRenderingDirectory, nullptr, LogFile::ATTACHED_PROJECT);
...
void CleanupRenderMess()
{
	...
	case LogFile::DURING_RENDER:
		...
		//if it failed here: the render completed but the avi failed to be transfered.  We need to attempt to move the file back to the hot folder.
